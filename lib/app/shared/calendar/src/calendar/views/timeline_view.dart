...
class _TimelineView extends CustomPainter {
  _TimelineView(this._horizontalLinesCountPerView, this._visibleDates, this._timeLabelWidth, this._timeSlotViewSettings,
      this._timeIntervalHeight, this._cellBorderColor, this._isDark);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _linePainter.color =
        _cellBorderColor != null ? _cellBorderColor : _isDark ? Colors.white70 : Colors.black.withOpacity(0.16);
    _x1 = 0;
    _x2 = size.width;
    _y1 = _timeLabelWidth;
    _y2 = _timeLabelWidth;
    ...
    for (int i = 0; i < _horizontalLinesCountPerView * _visibleDates.length; i++) {
      _y1 = _timeLabelWidth;
  ...
  void _drawTimeLabels(Canvas canvas, Size size, double xPosition) {
    ...
    timeTextStyle ??=
        TextStyle(color: _isDark ? Colors.white38 : Colors.black54, fontWeight: FontWeight.w500, fontSize: 10);
    ...
    for (int i = 0; i < _horizontalLinesCountPerView; i++) {
      ...
      date = DateTime(date.day, date.month, date.year, _startHour.toInt(), minute.toInt());
      ...
      _textPainter.paint(canvas, Offset(xPosition, _timeLabelWidth / 2 - _textPainter.height / 2));
  ...
  bool shouldRepaint(CustomPainter oldDelegate) {
    ...
    return oldWidget._horizontalLinesCountPerView != _horizontalLinesCountPerView ||
...
class _TimelineViewHeaderView extends CustomPainter {
  _TimelineViewHeaderView(this._visibleDates, this._calendarViewState, this._repaintNotifier, this._viewHeaderStyle,
      this._timeSlotViewSettings, this._viewHeaderHeight, this._isDark, this._todayHighlightColor)
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double scrolledPosition = _calendarViewState._timelineViewHeaderScrollController.offset;
    ...
    for (int i = 0; i < _visibleDates.length; i++) {
      ...
      String dayFormat = 'EE';
      dayFormat = dayFormat == _timeSlotViewSettings.dayFormat ? 'EEEE' : _timeSlotViewSettings.dayFormat;

      final String dayText = DateFormat(dayFormat).format(_currentDate).toString();
      final String dateText = DateFormat(_timeSlotViewSettings.dateFormat).format(_currentDate).toString();

      if (_isSameDate(_currentDate, _today)) {
        _dayTextStyle = _viewHeaderDayStyle.copyWith(color: _todayHighlightColor);
        _dateTextStyle = _viewHeaderDateStyle.copyWith(color: _todayHighlightColor);
      } else {
        _dateTextStyle = _viewHeaderDateStyle;
        _dayTextStyle = _viewHeaderDayStyle;
      }

      final TextSpan dayTextSpan = TextSpan(text: dayText, style: _dayTextStyle);
      ...
      final TextSpan dateTextSpan = TextSpan(text: dateText, style: _dateTextStyle);
      ...
      if (dateTextPainter.width + _xPosition + (_padding * 2) + dayTextPainter.width > (i + 1) * childWidth) {
        _xPosition = ((i + 1) * childWidth) - (dateTextPainter.width + (_padding * 2) + dayTextPainter.width);
      }

      dateTextPainter.paint(canvas, Offset(_padding + _xPosition, _viewHeaderHeight / 2 - dateTextPainter.height / 2));
      dayTextPainter.paint(
          canvas,
          Offset(
              dateTextPainter.width + _xPosition + (_padding * 2), _viewHeaderHeight / 2 - dayTextPainter.height / 2));
      if (index == i) {
