...
class _MonthCellPainter extends CustomPainter {
  _MonthCellPainter(this._visibleDates, this._rowCount, this._monthCellStyle, this._isDark, this._todayHighlightColor,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final DateTime _currentDate = _visibleDates[(_visibleDates.length / 2).truncate()];
    ...
    currentMonthTextStyle ??=
        TextStyle(fontSize: 13, fontFamily: 'Roboto', color: _isDark ? Colors.white : Colors.black87);
    previousMonthTextStyle ??=
        TextStyle(fontSize: 13, fontFamily: 'Roboto', color: _isDark ? Colors.white70 : Colors.black54);
    nextMonthTextStyle ??=
        TextStyle(fontSize: 13, fontFamily: 'Roboto', color: _isDark ? Colors.white70 : Colors.black54);
    ...
    for (int i = 0; i < _visibleDates.length; i++) {
      ...
      canvas.drawRect(Rect.fromLTWH(xPosition, yPosition - 5, cellWidth, cellHeight), _linePainter);
      ...
      if (_isCurrentDate) {
        ...
        canvas.drawCircle(Offset(xPosition + cellWidth / 2, yPosition + _textStyle.fontSize * 0.6),
            _textStyle.fontSize * 0.75, _circlePainter);
      }
    ...
    canvas.drawLine(const Offset(0, linePadding), Offset(size.width, linePadding), _linePainter);
