...
class _TimeRulerView extends CustomPainter {
  _TimeRulerView(this._horizontalLinesCount, this._timeIntervalHeight, this._timeSlotViewSettings,
      this._cellBorderColor, this._isDark);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(_width - offset, 0), Offset(_width - offset, size.height), _linePainter);
    ...
    final dynamic hour = (_timeSlotViewSettings.startHour - _timeSlotViewSettings.startHour.toInt()) * 60;
    double timeLabelPadding = 0;
    for (int i = 1; i <= _horizontalLinesCount; i++) {
      final dynamic minute = (i * _getTimeInterval(_timeSlotViewSettings)) + hour;
      date = DateTime(date.day, date.month, date.year, _timeSlotViewSettings.startHour.toInt(), minute.toInt());
      final dynamic _time = DateFormat(_timeSlotViewSettings.timeFormat).format(date).toString();
      ...
      _textPainter.paint(canvas, Offset(timeLabelPadding, y - (_textPainter.height / 2)));
