...
class _CalendarView extends StatefulWidget {
  const _CalendarView(this._calendar, this._visibleDates, this._width, this._height, this._agendaSelectedDate,
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (!_isTimelineView(widget._calendar.view) && widget._calendar.view != CalendarView.month) {
    ...
    _timeIntervalHeight = _getTimeIntervalHeight(
        widget._calendar, widget._width, widget._height, widget._visibleDates.length, _allDayHeight);
    if (widget._calendar.view != CalendarView.month) {
      _horizontalLinesCount = _getHorizontalLinesCount(widget._calendar.timeSlotViewSettings);
  ...
  void _scrollToPosition() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      if (_isWithInVisibleDateRange(widget._visibleDates, _updateCalendarStateDetails._currentDate)) {
        if (!_isTimelineView(widget._calendar.view)) {
          timeToPosition =
              _timeToPosition(widget._calendar, _updateCalendarStateDetails._currentDate, _timeIntervalHeight);
        } else {
          for (int i = 0; i < widget._visibleDates.length; i++) {
            if (_isSameDate(_updateCalendarStateDetails._currentDate, widget._visibleDates[i])) {
  ...
  @override
  void dispose() {
    if (_isTimelineView(widget._calendar.view) && _timelineViewAnimationController != null)
      _timelineViewAnimationController.dispose();
    if (_scrollController != null) {
      _scrollController.dispose();
    }
    if (_timelineViewHeaderScrollController != null) _timelineViewHeaderScrollController.dispose();
  ...
  void _scrollListener() {
    if (_isTimelineView(widget._calendar.view)) {
      ...
      if (_scrollController.position.atEdge &&
          _updateCalendarStateDetails._currentViewVisibleDates == widget._visibleDates) {
        setState(() {
          _scrollPhysics = const NeverScrollableScrollPhysics();
        });
      ...
      if (_timelineViewHeader != null) {
        _timelineViewHeader._repaintNotifier.value = !_timelineViewHeader._repaintNotifier.value;
      }
  ...
  bool _isUpdateTimelineViewScroll(double _initial, double _dx) {
    ...
    if (_scrollController.offset < _scrollController.position.maxScrollExtent &&
      ...
      _scrollToPosition = _initial - _dx <= _scrollController.position.maxScrollExtent
          ? _initial - _dx
      ...
      _scrollToPosition = _scrollController.position.maxScrollExtent - (_dx - _initial);
      _scrollToPosition = _scrollToPosition >= _scrollController.position.minScrollExtent
          ? _scrollToPosition
          : _scrollController.position.minScrollExtent;
      _scrollController.jumpTo(_scrollController.position.maxScrollExtent - (_dx - _initial));
      return true;
    }
  ...
  bool _isAnimateTimelineViewScroll(double _position, double _velocity) {
    ...
    _velocity = _velocity == 0 ? _position.abs() : _velocity / window.devicePixelRatio;
    if (_scrollController.offset < _scrollController.position.maxScrollExtent &&
        _scrollController.offset <= _scrollController.position.viewportDimension &&
        _position < 0) {
      setState(() {
      ...
      _timelineViewTween.end = _velocity.abs() > _scrollController.position.maxScrollExtent
          ? _scrollController.position.maxScrollExtent
          : _velocity.abs();

      if (_timelineViewAnimationController.isCompleted && _scrollController.offset != _timelineViewTween.end)
      ...
      _timelineViewTween.end =
          _scrollController.position.maxScrollExtent - _velocity.abs() < _scrollController.position.minScrollExtent
              ? _scrollController.position.minScrollExtent
  ...
  void didUpdateWidget(_CalendarView oldWidget) {
    if (widget._calendar.view != CalendarView.month) {
      ...
      if (widget._calendar.timeSlotViewSettings.startHour != oldWidget._calendar.timeSlotViewSettings.startHour ||
          widget._calendar.timeSlotViewSettings.endHour != oldWidget._calendar.timeSlotViewSettings.endHour ||
      ...
      _scrollController = _scrollController ?? ScrollController(initialScrollOffset: 0, keepScrollOffset: true)
        ..addListener(_scrollListener);
      ...
      if (_isTimelineView(widget._calendar.view)) {
        ...
        _timelineViewAnimationController = _timelineViewAnimationController ??
            AnimationController(
                duration: const Duration(milliseconds: 300), vsync: this, animationBehavior: AnimationBehavior.normal);
        _timelineViewTween = _timelineViewTween ?? Tween<double>(begin: 0.0, end: 0.1);

        _timelineViewAnimation = _timelineViewAnimation ?? _timelineViewTween.animate(_timelineViewAnimationController)
          ..addListener(_scrollAnimationListener);
  ...
  dynamic _updatePainterProperties(_UpdateCalendarStateDetails details) {
    ...
    details._allDayAppointmentViewCollection = _updateCalendarStateDetails._allDayAppointmentViewCollection;
    details._currentViewVisibleDates = _updateCalendarStateDetails._currentViewVisibleDates;
    details._visibleAppointments = _updateCalendarStateDetails._visibleAppointments;
  ...
  Widget _addAllDayAppointmentPanel(bool _isDark) {
    ...
    final double timeLabelWidth =
        _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view);
    double topPosition = _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
    if (widget._calendar.view == CalendarView.day) {
      topPosition = _allDayHeight;
    }

    if (_allDayHeight == 0 || widget._visibleDates != _updateCalendarStateDetails._currentViewVisibleDates) {
      return Positioned(left: 0, right: 0, top: topPosition, height: 1, child: _shadowView);
    }
    ...
    double _panelHeight = _updateCalendarStateDetails._allDayPanelHeight - _allDayHeight;
    if (_panelHeight < 0) {
      _panelHeight = 0;
    }

    _allDaySelectionNotifier?.value = null;
    final double _alldayExpanderHeight = _allDayHeight + (_panelHeight * _allDayExpanderAnimation.value);
    return Positioned(
      left: 0,
      top: topPosition,
      right: 0,
              ...
              children: <Widget>[
                CustomPaint(
                  painter: _AllDayAppointmentPainter(
                      ...
                      widget._visibleDates == _updateCalendarStateDetails._currentViewVisibleDates
                      ...
                      _updateCalendarStateDetails._allDayPanelHeight != _allDayHeight &&
                          (_heightAnimation.value == 1 || widget._calendar.view == CalendarView.day),
                      _allDayExpanderAnimation.value != 0.0 && _allDayExpanderAnimation.value != 1,
                      _isDark,
                      _allDaySelectionNotifier, updateCalendarState: (_UpdateCalendarStateDetails details) {
                    _updatePainterProperties(details);
                  }),
                  size: Size(widget._width, _updateCalendarStateDetails._allDayPanelHeight),
                ),
              ],
            ),
          ),
          Positioned(left: 0, top: _alldayExpanderHeight - 1, right: 0, height: 1, child: _shadowView),
        ],
      ),
    );
  }
  ...
  _AppointmentPainter _addAppointmentPainter() {
    ...
    _appointmentPainter = _AppointmentPainter(
        ...
        widget._visibleDates == _updateCalendarStateDetails._currentViewVisibleDates
            ...
            : null,
        _timeIntervalHeight,
        ValueNotifier<bool>(false), updateCalendarState: (_UpdateCalendarStateDetails details) {
      _updatePainterProperties(details);
    });
  ...
  Widget _addMonthView(bool _isDark) {
    final double viewHeaderHeight = _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
            ...
            child: RepaintBoundary(
              child: CustomPaint(
                painter: _ViewHeaderViewPainter(
                    ...
                    _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view),
                    _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view),
  ...
  Widget _addDayView(double width, double height, bool _isDark) {
    ...
    double viewHeaderHeight = _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
    final double timeLabelWidth =
        _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view);
    if (widget._calendar.view == null || widget._calendar.view == CalendarView.day) {
    ...
    final double _alldayExpanderHeight = _panelHeight * _allDayExpanderAnimation.value;
    return Stack(
      children: <Widget>[
        Positioned(
          left: 0,
          ...
          height: _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view),
          child: Container(
            color: widget._calendar.viewHeaderStyle.backgroundColor,
            child: RepaintBoundary(
              child: CustomPaint(
          ...
          child: ListView(
              ...
              children: <Widget>[
                Stack(children: <Widget>[
                  RepaintBoundary(
                    child: CustomPaint(
                      painter: _TimeSlotView(widget._visibleDates, _horizontalLinesCount, _timeIntervalHeight,
                          timeLabelWidth, widget._calendar.cellBorderColor, _isDark),
                      size: Size(width, height),
                    ),
                  ),
                  RepaintBoundary(
                    child: CustomPaint(
                      painter: _TimeRulerView(_horizontalLinesCount, _timeIntervalHeight,
                          widget._calendar.timeSlotViewSettings, widget._calendar.cellBorderColor, _isDark),
      ...
      Positioned(
        ...
        child: Container(
          ...
          child: _getTimelineViewHeader(
              width, _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view), _isDark),
        ),
      ),
      Positioned(
        top: _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view),
        left: 0,
        right: 0,
        bottom: 0,
        child: ListView(
  ...
  void _updateCalendarTapCallback(TapUpDetails details) {
    final double _viewHeaderHeight = _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
  ...
  void _updateCalendarTapCallbackForCalendarCells(TapUpDetails details) {
    ...
    if (widget._calendar.view != CalendarView.month && !_isTimelineView(widget._calendar.view)) {
      xPosition = details.localPosition.dx;
      yPosition = details.localPosition.dy;
    } else {
      yPosition =
          details.localPosition.dy - _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
    }
    _handleTouch(Offset(xPosition, yPosition), details);
  }

  // method to update the calendar tapped call back for the view header view
  void _updateCalendarTapCallbackForViewHeader(TapUpDetails details, double _width) {
    int index = 0;
    DateTime selectedDate;
    final double _timeLabelViewWidth =
        _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view);
    if (!_isTimelineView(widget._calendar.view)) {
      double cellWidth = 0;
      if (widget._calendar.view != CalendarView.month) {
        cellWidth = (_width - _timeLabelViewWidth) / widget._visibleDates.length;
        index = ((details.localPosition.dx - _timeLabelViewWidth) / cellWidth).truncate();
      } else {
      ...
      index =
          ((_scrollController.offset + details.localPosition.dx) / _getSingleViewWidthForTimeLineView(this)).truncate();

      selectedDate = widget._visibleDates[index];
    }

    _raiseCalendarTapCallback(widget._calendar, date: selectedDate, element: CalendarElement.viewHeader);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget._calendar.view == CalendarView.month) {
    ...
    } else if (!_isTimelineView(widget._calendar.view) && widget._calendar.view != CalendarView.month) {
      _allDayHeight = 0;

      if (widget._calendar.view == CalendarView.day) {
        final double viewHeaderHeight = _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
        if (widget._visibleDates == _updateCalendarStateDetails._currentViewVisibleDates) {
          _allDayHeight = _kAllDayLayoutHeight > viewHeaderHeight &&
                  _updateCalendarStateDetails._allDayPanelHeight > viewHeaderHeight
      ...
      if (widget._calendar.view != CalendarView.day &&
          widget._visibleDates == _updateCalendarStateDetails._currentViewVisibleDates) {
        _allDayHeight = _updateCalendarStateDetails._allDayPanelHeight > _kAllDayLayoutHeight
            ? _kAllDayLayoutHeight
            : _updateCalendarStateDetails._allDayPanelHeight;
      ...
      return GestureDetector(
        child: _addDayView(widget._width, _timeIntervalHeight * _horizontalLinesCount, _isDark),
        onTapUp: (TapUpDetails details) {
          _updateCalendarTapCallback(details);
        },
      );
      ...
      return GestureDetector(
        child: _addTimelineView(
            _timeIntervalHeight * (_horizontalLinesCount * widget._visibleDates.length), widget._height, _isDark),
        onTapUp: (TapUpDetails details) {
          _updateCalendarTapCallback(details);
        },
      );
  ...
  _AppointmentView _getAppointmentOnPoint(List<_AppointmentView> _appointmentCollection, double x, double y) {
  ...
  void _handleTouch(Offset details, TapUpDetails tapUpDetails) {
    ...
    if (widget._calendar.view != CalendarView.month && !_isTimelineView(widget._calendar.view)) {
      _timeLabelWidth = _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view);
      final double _viewHeaderHeight = widget._calendar.view == CalendarView.day
          ? 0
          : _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view);
      final double allDayHeight = isExpanded ? _updateCalendarStateDetails._allDayPanelHeight : _allDayHeight;
      if (details.dx <= _timeLabelWidth && details.dy > _viewHeaderHeight + allDayHeight) {
        return;
      }
      ...
      if (details.dy < _viewHeaderHeight) {
        if (details.dx > _timeLabelWidth) {
          if (_shouldRaiseCalendarTapCallback(widget._calendar.onTap)) {
            _updateCalendarTapCallbackForViewHeader(tapUpDetails, widget._width);
          }
        ...
        if (widget._calendar.view == CalendarView.day &&
            ...
            details.dy < _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view)) {
          if (_shouldRaiseCalendarTapCallback(widget._calendar.onTap)) {
            _updateCalendarTapCallbackForViewHeader(tapUpDetails, widget._width);
          }
          return;
        ...
        final _AppointmentView _appointmentView =
            _getAppointmentOnPoint(_updateCalendarStateDetails._allDayAppointmentViewCollection, details.dx, yPosition);
        if (_appointmentView != null &&
          ...
          _selectionPainter._repaintNotifier.value = !_selectionPainter._repaintNotifier.value;
        ...
        } else if (_updateCalendarStateDetails._allDayPanelHeight > allDayHeight &&
            yPosition > allDayHeight - _kAllDayAppointmentHeight) {
          _expandOrCollapseAllDay();
          return;
        }
      } else {
        final double yPosition = details.dy - _viewHeaderHeight - allDayHeight + _scrollController.offset;
        final _AppointmentView _appointmentView =
            _getAppointmentOnPoint(_appointmentPainter._appointmentCollection, details.dx, yPosition);
        _allDaySelectionNotifier?.value = null;
        if (_appointmentView == null) {
          _drawSelection(details.dx - _timeLabelWidth, details.dy - _viewHeaderHeight - allDayHeight, _timeLabelWidth);
        } else {
          if (_selectedDate != null) {
            _selectedDate = null;
            _selectionPainter._selectedDate = _selectedDate;
      ...
      if (details.dy < _getTimeLabelWidth(widget._calendar.timeSlotViewSettings.timeRulerSize, widget._calendar.view)) {
        return;
      }
      ...
      final _AppointmentView _appointmentView =
          _getAppointmentOnPoint(_appointmentPainter._appointmentCollection, xPosition, yPosition);
      if (_appointmentView == null) {
        _drawSelection(
            details.dx,
            details.dy - _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view),
      ...
      if (_selectionPainter._selectedDate != null) {
        ...
        if (widget._calendar.view == CalendarView.month && !_isTimelineView(widget._calendar.view)) {
          _selectedAppointments =
              (widget._calendar.dataSource != null && !_isCalendarAppointment(widget._calendar.dataSource.appointments))
                  ? _getCustomAppointments(_getSelectedDateAppointments(_updateCalendarStateDetails._appointments,
                      widget._calendar.timeZone, _selectionPainter._selectedDate))
                  : (_getSelectedDateAppointments(_updateCalendarStateDetails._appointments, widget._calendar.timeZone,
        ...
        _selectedAppointments = <dynamic>[_selectedAppointment._data ?? _selectedAppointment];
        _raiseCalendarTapCallback(widget._calendar,
  ...
  void _drawSelection(double x, double y, double _timeLabelWidth) {
    ...
    if (widget._calendar.view == CalendarView.month) {
      ...
      cellHeight = (widget._height - _getViewHeaderHeight(widget._calendar.viewHeaderHeight, widget._calendar.view)) /
          widget._calendar.monthViewSettings.numberOfWeeksInView;
      ...
      final dynamic time = ((_getTimeInterval(widget._calendar.timeSlotViewSettings) / 60) * columnIndex) +
          widget._calendar.timeSlotViewSettings.startHour;
      final int hour = time.toInt();
      final dynamic minute = ((time - hour) * 60).round();
      _selectedDate = DateTime(
          widget._visibleDates[0].year, widget._visibleDates[0].month, widget._visibleDates[0].day, hour, minute);
      _updateCalendarStateDetails._selectedDate = _selectedDate;
      _updateCalendarStateDetails._isAppointmentTapped = false;
    } else if (widget._calendar.view == CalendarView.workWeek || widget._calendar.view == CalendarView.week) {
      if (y >= _timeIntervalHeight * _horizontalLinesCount) {
        return;
      }
      ...
      if (x >= _timeIntervalHeight * (_horizontalLinesCount * widget._visibleDates.length)) {
        return;
      }
      ...
      final dynamic time = ((_getTimeInterval(widget._calendar.timeSlotViewSettings) / 60) * rowIndex) +
          widget._calendar.timeSlotViewSettings.startHour;
      final int hour = time.toInt();
      final dynamic minute = ((time - hour) * 60).round();
