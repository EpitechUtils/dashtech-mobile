...
class _SelectionPainter extends CustomPainter {
  _SelectionPainter(this._calendar, this._visibleDates, this._selectedDate, this._selectionDecoration,
      this._timeIntervalHeight, this._repaintNotifier,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (!_isTimeline) {
      if (_calendar.view == CalendarView.month) {
        ...
        _cellHeight = size.height / _calendar.monthViewSettings.numberOfWeeksInView;
      } else {
    ...
    if (!_isTimeline) {
      if (_calendar.view == CalendarView.month) {
        ...
        if (_appointmentView != null && _appointmentView.appointment != null) {
          ...
          if (_isSameDate(_visibleDates[0], _selectedDate)) {
            ...
            _yPosition = _timeToPosition(_calendar, _selectedDate, _timeIntervalHeight);
      ...
      if (_calendar.view == CalendarView.timelineDay) {
        if (_appointmentView != null && _appointmentView.appointment != null) {
          ...
          if (_isSameDate(_visibleDates[0], _selectedDate)) {
            _xPosition = _timeToPosition(_calendar, _selectedDate, _timeIntervalHeight);
            _yPosition = _getTimeLabelWidth(_calendar.timeSlotViewSettings.timeRulerSize, _calendar.view);
            _drawSlotSelection(_width, size.height, canvas);
          }
        }
      } else {
          ...
          if (_isWithInVisibleDateRange(_visibleDates, _selectedDate)) {
            for (int i = 0; i < _visibleDates.length; i++) {
              if (_isSameDate(_selectedDate, _visibleDates[i])) {
                ...
                _rowIndex = i;
                _xPosition =
                    (_rowIndex * singleViewWidth) + _timeToPosition(_calendar, _selectedDate, _timeIntervalHeight);
                _yPosition = _getTimeLabelWidth(_calendar.timeSlotViewSettings.timeRulerSize, _calendar.view);
                _drawSlotSelection(_width, size.height, canvas);
                break;
              }
  ...
  void _drawSlotSelection(double _width, double height, Canvas canvas) {
    ...
    final Rect rect = Rect.fromLTRB(
        ...
        _xPosition + _cellWidth == _width ? _xPosition + _cellWidth - padding : _xPosition + _cellWidth,
