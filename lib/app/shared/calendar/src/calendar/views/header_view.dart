...
class _HeaderViewPainter extends CustomPainter {
  _HeaderViewPainter(
      this._visibleDates, this._headerStyle, this._currentDate, this._view, this._numberOfWeeksInView, this._isDark);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_view == CalendarView.month &&
        ...
        _visibleDates[0].month != _visibleDates[_visibleDates.length - 1].month) {
      _headerText = DateFormat('MMM yyyy').format(_visibleDates[0]).toString() +
          ' - ' +
          DateFormat('MMM yyyy').format(_visibleDates[_visibleDates.length - 1]).toString();
    ...
    if (_headerStyle.textAlign == TextAlign.right || _headerStyle.textAlign == TextAlign.end) {
    ...
    _textPainter.paint(canvas, Offset(_xPosition, size.height / 2 - _textPainter.height / 2));
  }
