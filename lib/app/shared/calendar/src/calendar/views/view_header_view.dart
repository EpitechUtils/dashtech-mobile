...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double _labelWidth = _view == CalendarView.day && _timeLabelWidth < 50 ? 50 : _timeLabelWidth;
    ...
    if (_view != CalendarView.month) {
      ...
      if (_view == CalendarView.day) {
        ...
        if (_timeLabelWidth == _labelWidth) {
          canvas.drawLine(Offset(_labelWidth - 0.5, 0), Offset(_labelWidth - 0.5, size.height), _linePainter);
        }
      }
      ...
      for (int i = 0; i < _visibleDates.length; i++) {
        ...
        _dayText = DateFormat(_timeSlotViewSettings.dayFormat).format(_currentDate).toString().toUpperCase();

        _updateViewHeaderFormat();

        _dateText = DateFormat(_timeSlotViewSettings.dateFormat).format(_currentDate).toString();
        final bool _isToday = _isSameDate(_currentDate, today);
        if (_isToday) {
          dayTextStyle = _viewHeaderDayStyle.copyWith(color: _todayHighlightColor);
        ...
        _yPosition = size.height / 2 - (_dayTextPainter.height + topPadding + _dateTextPainter.height) / 2;

        _dayTextPainter.paint(canvas, Offset(_xPosition + dayXPosition, _yPosition));
        if (_isToday) {
          _drawTodayCircle(
              canvas, _xPosition + dateXPosition, _yPosition + topPadding + _dayTextPainter.height, _dateTextPainter);
        }

        _dateTextPainter.paint(
            canvas, Offset(_xPosition + dateXPosition, _yPosition + topPadding + _dayTextPainter.height));
      ...
      for (int i = 0; i < 7; i++) {
        _currentDate = _visibleDates[i];
        _dayText = DateFormat(_monthViewSettings.dayFormat).format(_currentDate).toString().toUpperCase();

        _updateViewHeaderFormat();

        _hasToday = _monthViewSettings.numberOfWeeksInView > 0 && _monthViewSettings.numberOfWeeksInView < 6
            ? true
            : _visibleDates[_visibleDates.length ~/ 2].month == today.month ? true : false;

        if (_hasToday && _isWithInVisibleDateRange(_visibleDates, today) && _currentDate.weekday == today.weekday) {
          dayTextStyle = _viewHeaderDayStyle.copyWith(color: _todayHighlightColor);
        } else {
          dayTextStyle = _viewHeaderDayStyle;
        }
        ...
        _dayTextPainter.paint(canvas, Offset(_xPosition + (_width / 2 - _dayTextPainter.width / 2), _yPosition));
        _xPosition += _width;
      }
    }
  }
  ...
  void _drawTodayCircle(Canvas canvas, double x, double y, TextPainter dateTextPainter) {
    ...
    canvas.drawCircle(Offset(x + painterWidth, y + painterHeight), radius + _circlePadding, _circlePainter);
  }
}
