...
class _AgendaViewPainter extends CustomPainter {
  _AgendaViewPainter(this._monthViewSettings, this._selectedDate, this._timeZone, this._appointments);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_selectedDate != null) {
      agendaAppointments = _getSelectedDateAppointments(_appointments, _timeZone, _selectedDate);
    }

    if (_selectedDate == null || agendaAppointments == null || agendaAppointments.isEmpty) {
    ...
    TextStyle appointmentTextStyle = _monthViewSettings.agendaStyle.appointmentTextStyle;
    appointmentTextStyle ??= TextStyle(color: Colors.white, fontSize: 13, fontFamily: 'Roboto');
    //// Draw Appointments
    for (int i = 0; i < agendaAppointments.length; i++) {
      ...
      final double appointmentHeight =
          _appointment.isAllDay ? _monthViewSettings.agendaItemHeight / 2 : _monthViewSettings.agendaItemHeight;
      final Rect rect = Rect.fromLTWH(xPosition, yPosition, size.width - xPosition - padding, appointmentHeight);
      final Radius cornerRadius = Radius.circular((appointmentHeight * 0.1) > 5 ? 5 : (appointmentHeight * 0.1));
      canvas.drawRRect(RRect.fromRectAndRadius(rect, cornerRadius), _rectPainter);

      TextSpan span = TextSpan(text: _appointment.subject, style: appointmentTextStyle);
      ...
      if (!_appointment.isAllDay) {
        painter.layout(minWidth: 0, maxWidth: size.width - (2 * padding) - xPosition);
        painter.paint(canvas, Offset(xPosition + padding, yPosition + (appointmentHeight / 2) - painter.height));

        final String format =
            _isSameDate(_appointment._actualStartTime, _appointment._actualEndTime) ? 'hh:mm a' : 'MMM dd, hh:mm a';
        span = TextSpan(
        ...
        painter.layout(minWidth: 0, maxWidth: size.width - (2 * padding) - xPosition);
        painter.paint(canvas, Offset(xPosition + padding, yPosition + (appointmentHeight / 2)));
      } else {
        //// Draw All day appointment
        painter.layout(minWidth: 0, maxWidth: size.width - 10 - xPosition);
        painter.paint(canvas, Offset(xPosition + 5, yPosition + ((appointmentHeight - painter.height) / 2)));
      }
      ...
      if (_appointment.recurrenceRule != null && _appointment.recurrenceRule.isNotEmpty) {
        ...
        final TextSpan icon = _getRecurrenceIcon(appointmentTextStyle.color, textSize);
        painter.text = icon;
        painter.layout(minWidth: 0, maxWidth: size.width - (2 * padding) - xPosition);
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromLTRB(rect.right - textSize - 8, rect.top, rect.right, rect.bottom), cornerRadius),
            _rectPainter);
        // Value 8 added as a right side padding for the recurrence icon in the agenda view
        painter.paint(canvas, Offset(rect.right - textSize - 8, rect.top + (rect.height - painter.height) / 2));
      }
...
class _AgendaDateTimePainter extends CustomPainter {
  _AgendaDateTimePainter(this._selectedDate, this._monthViewSettings, this._isDark, this._todayHighlightColor);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Color selectedDayTextColor = isToday
        ...
        : dayTextStyle != null ? dayTextStyle.color : _isDark ? Colors.white70 : Colors.black54;
    final Color selectedDateTextColor =
        isToday ? Colors.white : dateTextStyle != null ? dateTextStyle.color : _isDark ? Colors.white : Colors.black;
    dayTextStyle = dayTextStyle != null
        ? dayTextStyle.copyWith(color: selectedDayTextColor)
        : TextStyle(color: selectedDayTextColor, fontWeight: FontWeight.w500, fontSize: 10, fontFamily: 'Roboto');
    dateTextStyle = dateTextStyle != null
        ? dateTextStyle.copyWith(color: selectedDateTextColor)
        : TextStyle(color: selectedDateTextColor, fontSize: 20, fontFamily: 'Roboto', fontWeight: FontWeight.normal);

    //// Draw Weekday
    TextSpan span =
        TextSpan(text: DateFormat('EEE').format(_selectedDate).toUpperCase().toString(), style: dayTextStyle);
    _textPainter = _textPainter ?? TextPainter();
    _textPainter.text = span;
    _textPainter.maxLines = 1;
    _textPainter.textDirection = TextDirection.ltr;
