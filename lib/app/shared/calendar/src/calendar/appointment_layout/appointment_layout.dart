...
class _AppointmentPainter extends CustomPainter {
  _AppointmentPainter(
      this._calendar, this._visibleDates, this._visibleAppointments, this._timeIntervalHeight, this._repaintNotifier,
  ...
  final _UpdateCalendarStateDetails _updateCalendarStateDetails = _UpdateCalendarStateDetails();
  ...
  bool shouldRepaint(CustomPainter oldDelegate) {
    ...
    if (oldWidget._visibleDates != _visibleDates || oldWidget._visibleAppointments != _visibleAppointments) {
      return true;
    }
  ...
  void _drawMonthAppointment(Canvas canvas, Size size, Paint paint) {
    ...
    final double cellHeight = size.height / _calendar.monthViewSettings.numberOfWeeksInView;
    double xPosition = 0;
    double yPosition = 0;
    if (_calendar.monthViewSettings.appointmentDisplayMode == MonthAppointmentDisplayMode.none) {
      return;
    } else if (_calendar.monthViewSettings.appointmentDisplayMode == MonthAppointmentDisplayMode.appointment) {
      ...
      final double fontSize = _calendar.monthViewSettings.monthCellStyle.todayTextStyle.fontSize;
      //// cell top padding and start position calculated by day view rendering padding and size.
      const double cellTopPadding = 6;
      final double startPosition = cellTopPadding + fontSize * 0.6 + fontSize * 0.75;
      final int maximumDisplayCount = _calendar.monthViewSettings.appointmentDisplayCount ?? 3;
      final double _appointmentHeight = (cellHeight - startPosition) / maximumDisplayCount;
      double textSize = -1;
      for (int i = 0; i < _appointmentCollection.length; i++) {
        ...
        if (_appointmentView.position <= maximumDisplayCount) {
          yPosition = yPosition + startPosition + (_appointmentHeight * (_appointmentView.position - 1));
        } else {
          yPosition = yPosition + startPosition + (_appointmentHeight * (maximumDisplayCount - 1));
        }

        final double _appointmentWidth = (_appointmentView.endIndex - _appointmentView.startIndex + 1) * cellWidth;
        ...
        if (_appointmentView.position == maximumDisplayCount) {
          ...
          for (int count = _appointmentView.startIndex; count <= _appointmentView.endIndex; count++) {
            for (int j = 0; j < 3; j++) {
              canvas.drawCircle(Offset(startXPosition, yPosition + (_appointmentHeight / 2)), radius, paint);
          ...
          if (textSize == -1) {
            for (double j = style.fontSize - 1; j > 0; j--) {
              //// left and right side padding value 2 subtracted in appointment width
              painter.layout(minWidth: 0, maxWidth: _appointmentWidth - 2 > 0 ? _appointmentWidth - 2 : 0);
            ...
            span = TextSpan(text: _appointment.subject, style: style.copyWith(fontSize: textSize));
            painter = TextPainter(
                text: span,
                maxLines: 1,
                textDirection: TextDirection.ltr,
          ...
          final Radius cornerRadius = Radius.circular((_appointmentHeight * 0.1) > 2 ? 2 : (_appointmentHeight * 0.1));
          ...
          final RRect rect = RRect.fromRectAndRadius(
              Rect.fromLTWH(
                  xPosition,
                  yPosition,
                  _appointmentWidth - rightSidePadding > 0 ? _appointmentWidth - rightSidePadding : 0,
          ...
          painter.layout(
              minWidth: 0,
              maxWidth: _appointmentWidth - rightSidePadding + 1 > 0 ? _appointmentWidth - rightSidePadding + 1 : 0);
          yPosition = yPosition + ((_appointmentHeight - 1 - painter.height) / 2);
          painter.paint(canvas, Offset(xPosition + 2, yPosition));

          if (_appointment.recurrenceRule != null && _appointment.recurrenceRule.isNotEmpty) {
            ...
            canvas.drawRRect(
                RRect.fromRectAndRadius(
                    Rect.fromLTRB(rect.right - textSize, yPosition, rect.right, rect.bottom), cornerRadius),
                paint);
      ...
      for (int i = 0; i < _visibleDates.length; i++) {
        final List<Appointment> appointmentLists =
            _getVisibleSelectedDateAppointments(_calendar, _visibleDates[i], _visibleAppointments);
        appointmentLists
        ...
        final int count = appointmentLists.length <= _calendar.monthViewSettings.appointmentDisplayCount
  ...
  void _drawDayAppointments(Canvas canvas, Size size, Paint paint) {
    final double _timeLabelWidth = _getTimeLabelWidth(_calendar.timeSlotViewSettings.timeRulerSize, _calendar.view);
    final double _width = size.width - _timeLabelWidth;
    _setAppointmentPositionAndMaxPosition(this, _calendar, _visibleAppointments, false, _timeIntervalHeight);
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      if (_column == -1 ||
          ...
          (_appointment.endTime.difference(_appointment.startTime).inDays > 0) ||
          _appointment.isAllDay) {
        continue;
      }

      final int totalHours = _appointment._actualStartTime.hour - _calendar.timeSlotViewSettings.startHour.toInt();
      final double mins = _appointment._actualStartTime.minute -
          (_calendar.timeSlotViewSettings.startHour - _calendar.timeSlotViewSettings.startHour.toInt());
      final int totalMins = (totalHours * 60 + mins).toInt();
      final dynamic _row = totalMins ~/ _getTimeInterval(_calendar.timeSlotViewSettings);

      final double appointmentWidth = cellWidth / _appointmentView.maxPositions;
      xPosition = _column * cellWidth + (_appointmentView.position * appointmentWidth) + _timeLabelWidth;

      yPosition = _row * cellHeight;

      Duration difference = _appointment._actualEndTime.difference(_appointment._actualStartTime);
      final double minuteHeight = cellHeight / _getTimeInterval(_calendar.timeSlotViewSettings);
      yPosition += ((_appointment._actualStartTime.hour * 60 + _appointment._actualStartTime.minute) %
      ...
      if (_calendar.timeSlotViewSettings.minimumAppointmentDuration != null &&
          _calendar.timeSlotViewSettings.minimumAppointmentDuration.inMinutes > 0) {
        if (difference < _calendar.timeSlotViewSettings.minimumAppointmentDuration &&
            difference.inMinutes * minuteHeight < _calendar.timeSlotViewSettings.timeIntervalHeight) {
          difference = _calendar.timeSlotViewSettings.minimumAppointmentDuration;
      ...
      paint.color = _appointment.color;
      final RRect rect =
          RRect.fromRectAndRadius(Rect.fromLTWH(xPosition, yPosition, appointmentWidth - 1, height - 1), cornerRadius);
      _appointmentView.appointmentRect = rect;
      canvas.drawRRect(rect, paint);
      ...
      painter.paint(canvas, Offset(xPosition + textPadding, yPosition + textPadding));
      if (_appointment.recurrenceRule != null && _appointment.recurrenceRule.isNotEmpty) {
        ...
        final TextSpan icon = _getRecurrenceIcon(_calendar.appointmentTextStyle.color, textSize);
        painter.text = icon;
        painter.layout(minWidth: 0, maxWidth: appointmentWidth - textPadding - 2);
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromLTRB(rect.right - textSize - 1, rect.bottom - 2 - textSize, rect.right, rect.bottom),
                cornerRadius),
            paint);
        painter.paint(canvas, Offset(rect.right - textSize - 1, rect.bottom - 2 - textSize));
      }
    }
  }
  ...
  void _drawTimelineAppointments(Canvas canvas, Size size, Paint paint) {
    ...
    double xPosition = 0;
    double yPosition = 0;
    final double topPosition = _getTimeLabelWidth(_calendar.timeSlotViewSettings.timeRulerSize, _calendar.view);
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      if (_column == -1 && _appointment._actualStartTime.isBefore(_visibleDates[0])) {
        _column = 0;
      }
      ...
      if (_calendar.view == CalendarView.timelineWorkWeek) {
        ...
        for (int j = 0; j < count; j++) {
          ...
          if (_date != null && _date.day == endTime.day && _date.month == endTime.month && _date.year == endTime.year) {
        ...
        if (_endColumn == -1 && _appointment._actualEndTime.isAfter(_visibleDates[_visibleDates.length - 1])) {
      ...
      int totalHours = startTime.hour - _calendar.timeSlotViewSettings.startHour.toInt();
      ...
      final double minuteHeight = cellWidth / _getTimeInterval(_calendar.timeSlotViewSettings);

      double appointmentHeight = _calendar.timeSlotViewSettings.timelineAppointmentHeight;
      if (appointmentHeight * _appointmentView.maxPositions > size.height - topPosition) {
        appointmentHeight = (size.height - topPosition) / _appointmentView.maxPositions;
      }

      xPosition = (_column * viewWidth) + (_row * cellWidth);
      yPosition = topPosition + (appointmentHeight * _appointmentView.position);
      xPosition +=
          ((startTime.hour * 60 + startTime.minute) % _getTimeInterval(_calendar.timeSlotViewSettings)) * minuteHeight;

      paint.color = _appointment.color;
      double width = 0;
      if (_calendar.view == CalendarView.timelineWorkWeek) {
        totalHours = endTime.hour - _calendar.timeSlotViewSettings.startHour.toInt();
        mins = endTime.minute -
            (_calendar.timeSlotViewSettings.startHour - _calendar.timeSlotViewSettings.startHour.toInt());
        ...
        final double endXPosition = (_endColumn * viewWidth) +
            (_row * cellWidth) +
            ((endTime.hour * 60 + endTime.minute) % _getTimeInterval(_calendar.timeSlotViewSettings)) * minuteHeight;
      ...
      if (_calendar.timeSlotViewSettings.minimumAppointmentDuration != null &&
        ...
        final double minWidth = _getAppointmentHeightFromDuration(
            _calendar.timeSlotViewSettings.minimumAppointmentDuration, _calendar, _timeIntervalHeight);
        width = width > minWidth ? width : minWidth;
      }

      final Radius cornerRadius = Radius.circular((appointmentHeight * 0.1) > 2 ? 2 : (appointmentHeight * 0.1));
      final RRect rect =
          RRect.fromRectAndRadius(Rect.fromLTWH(xPosition, yPosition, width - 1, appointmentHeight - 1), cornerRadius);
      _appointmentView.appointmentRect = rect;
      canvas.drawRRect(rect, paint);
      final TextSpan span = TextSpan(
        text: _appointment.subject,
      ...
      //// left and right side padding value 2 subtracted in appointment width
      painter.layout(minWidth: 0, maxWidth: width - textPadding - 2);
      if (painter.maxLines == 1 && painter.height > appointmentHeight - textPadding - 2) {
        continue;
      }
