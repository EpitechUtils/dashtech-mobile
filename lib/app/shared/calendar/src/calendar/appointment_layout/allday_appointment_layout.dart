...
class _AllDayAppointmentPainter extends CustomPainter {
  _AllDayAppointmentPainter(this._calendar, this._visibleDates, this._visibleAppointment, this._timeLabelWidth,
      this._allDayPainterHeight, this._isExpandable, this._isExpanding, this._isDark, this._repaintNotifier,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _updateCalendarStateDetails._allDayAppointmentViewCollection =
        _updateCalendarStateDetails._allDayAppointmentViewCollection ?? <_AppointmentView>[];
    ...
    if (_calendar.view == CalendarView.day) {
      ...
      canvas.drawLine(Offset(_timeLabelWidth - 0.5, 0), Offset(_timeLabelWidth - 0.5, size.height), _linePainter);
    }
    ...
    int maxPosition = 0;
    if (_updateCalendarStateDetails._allDayAppointmentViewCollection.isNotEmpty) {
    ...
    for (int i = 0; i < _updateCalendarStateDetails._allDayAppointmentViewCollection.length; i++) {
      final _AppointmentView _appointmentView = _updateCalendarStateDetails._allDayAppointmentViewCollection[i];
      ...
      final RRect rect = RRect.fromRectAndRadius(
          Rect.fromLTRB(
              ...
              (_kAllDayAppointmentHeight * _appointmentView.position).toDouble(),
              (_appointmentView.endIndex * cellWidth) + _timeLabelWidth - textPadding,
              ((_kAllDayAppointmentHeight * _appointmentView.position) + _kAllDayAppointmentHeight - 1).toDouble()),
          const Radius.circular((_kAllDayAppointmentHeight * 0.1) > 2 ? 2 : (_kAllDayAppointmentHeight * 0.1)));

      _appointmentView.appointmentRect = rect;
      if (rect.left < _timeLabelWidth - 1 ||
          rect.right > size.width + 1 ||
          (rect.bottom > _allDayPainterHeight - _kAllDayAppointmentHeight && maxPosition > _position)) {
        continue;
      }
      ...
      _textPainter.paint(canvas, Offset(rect.left + textPadding, rect.top + (rect.height - _textPainter.height) / 2));
      if (_appointment.recurrenceRule != null && _appointment.recurrenceRule.isNotEmpty) {
        ...
        canvas.drawRRect(
            RRect.fromRectAndRadius(
                Rect.fromLTRB(rect.right - textSize, rect.top, rect.right, rect.bottom), rect.brRadius),
            _rectPainter);
        _textPainter.paint(
            canvas, Offset(rect.right - textSize - 1, rect.top + (rect.height - _textPainter.height) / 2));
      }
    ...
    if (_isExpandable && maxPosition > _position && !_isExpanding) {
      ...
      final double endYPosition = _allDayPainterHeight - _kAllDayAppointmentHeight;
      for (int i = 0; i < _visibleDates.length; i++) {
        int count = 0;
        final int leftPosition = startXPosition.toInt();
        final int rightPostion = endXPosition.toInt();
        for (_AppointmentView view in _updateCalendarStateDetails._allDayAppointmentViewCollection) {
          ...
          if (((rectLeftPosition >= leftPosition && rectLeftPosition < rightPostion) ||
                  (rectRightPosition > leftPosition && rectRightPosition < rightPostion) ||
                  (rectLeftPosition <= leftPosition && rectRightPosition > rightPostion)) &&
        ...
        _textPainter.text = span;
        _textPainter.layout(minWidth: 0, maxWidth: cellWidth - textPadding >= 0 ? cellWidth - textPadding : 0);

        _textPainter.paint(
            canvas,
            Offset(_timeLabelWidth + (i * cellWidth) + textPadding,
                endYPosition + ((_kAllDayAppointmentHeight - _textPainter.height) / 2)));
        startXPosition += cellWidth;
        endXPosition += cellWidth;
      }
    }
    ...
    if (_isExpandable) {
      ...
      _expanderTextPainter ??= TextPainter(textDirection: TextDirection.ltr, textAlign: TextAlign.left, maxLines: 1);
