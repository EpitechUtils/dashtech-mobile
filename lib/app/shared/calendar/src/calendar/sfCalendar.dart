...
typedef _UpdateCalendarState = void Function(_UpdateCalendarStateDetails _updateCalendarStateDetails);
...
class SfCalendar extends StatefulWidget {
  SfCalendar({
    ...
    TextStyle appointmentTextStyle,
            ...
            TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w500, fontFamily: 'Roboto'),
...
class _SfCalendarState extends State<SfCalendar> {
  ...
  List<_AppointmentView> _allDayAppointmentViewCollection = <_AppointmentView>[];
  ...
  Future<bool> _loadDataBase() async {
    final dynamic byteData = await rootBundle.load('packages/timezone/data/2019c.tzf');
  ...
  void initState() {
    ...
    _currentDate = _getCurrentDate(_minDate, _maxDate, widget.initialDisplayDate);
    _updateCurrentVisibleDates();
    widget.dataSource?.addListener(_dataSourceChangedListener);
    if (widget.view == CalendarView.month && widget.monthViewSettings.showAgenda) {
  ...
  void _updateCurrentVisibleDates() {
    ...
    _currentViewVisibleDates = _getVisibleDates(currentDate, _nonWorkingDays, widget.firstDayOfWeek,
        _getViewDatesCount(widget.view, widget.monthViewSettings.numberOfWeeksInView, currentDate));
  }
    ...
    if (type == CalendarDataSourceAction.reset) {
      ...
      setState(() {
        ...
        if (type == CalendarDataSourceAction.add) {
          final List<Appointment> collection = _generateCalendarAppointments(widget.dataSource, widget, data);
  ...
  void didUpdateWidget(SfCalendar oldWidget) {
    ...
    if (oldWidget.view != widget.view ||
        widget.monthViewSettings.numberOfWeeksInView != oldWidget.monthViewSettings.numberOfWeeksInView) {
    ...
    if (oldWidget.initialDisplayDate != widget.initialDisplayDate && widget.initialDisplayDate != null) {
      _currentDate = _getCurrentDate(_minDate, _maxDate, widget.initialDisplayDate);
    }

    if (widget.view == CalendarView.month && widget.monthViewSettings.showAgenda && _agendaScrollController == null) {
      _agendaScrollController = ScrollController(initialScrollOffset: 0, keepScrollOffset: true);
    }
  ...
  DateTime _updateCurrentDate(SfCalendar _oldWidget) {
    ...
    if (_selectedDate != null && _isWithInVisibleDateRange(_currentViewVisibleDates, _selectedDate)) {
      if (_oldWidget.view == CalendarView.month) {
        return DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day, widget.initialDisplayDate.hour,
            widget.initialDisplayDate.minute, widget.initialDisplayDate.second);
      } else {
        return _selectedDate;
      }
    } else if (_currentDate.month == DateTime.now().month && _currentDate.year == DateTime.now().year) {
      return DateTime.now();
    } else {
      if (_oldWidget.view == CalendarView.month) {
        ...
        return DateTime(_currentDate.year, _currentDate.month, 1, widget.initialDisplayDate.hour,
            widget.initialDisplayDate.minute, widget.initialDisplayDate.second);
      } else {
        return _currentViewVisibleDates[0];
      }
  ...
  void _updateAllDayAppointment() {
    ...
    if (widget.view != CalendarView.month && !_isTimelineView(widget.view) && _currentViewVisibleDates.length <= 7) {
      _allDayAppointmentViewCollection = _allDayAppointmentViewCollection ?? <_AppointmentView>[];
      ...
      for (Appointment apppointment in _visibleAppointments) {
        if (apppointment.isAllDay || apppointment._actualEndTime.difference(apppointment._actualStartTime).inDays > 0) {
      ...
      for (_AppointmentView _appointmentView in _allDayAppointmentViewCollection) {
        if (_appointmentView.appointment == null) {
          continue;
        }

        final int startIndex = _getIndex(_currentViewVisibleDates, _appointmentView.appointment._actualStartTime);
        final int endIndex = _getIndex(_currentViewVisibleDates, _appointmentView.appointment._actualEndTime) + 1;
      ...
      _allDayAppointmentViewCollection.sort((_AppointmentView app1, _AppointmentView app2) {
        if (app1.appointment != null && app2.appointment != null) {
          return (app2.endIndex - app2.startIndex) > (app1.endIndex - app1.startIndex) ? 1 : 0;
        }
      ...
      final List<List<_AppointmentView>> _allDayAppointmentView = <List<_AppointmentView>>[];
      //// Calculate the intersecting appointment view collection.
      for (int i = 0; i < _currentViewVisibleDates.length; i++) {
        final List<_AppointmentView> intersectingAppointments = <_AppointmentView>[];
        for (int j = 0; j < _allDayAppointmentViewCollection.length; j++) {
          final _AppointmentView _currentView = _allDayAppointmentViewCollection[j];
      ...
      for (int i = 0; i < _allDayAppointmentView.length; i++) {
        final List<_AppointmentView> intersectingAppointments = _allDayAppointmentView[i];
        for (int j = 0; j < intersectingAppointments.length; j++) {
          ...
          if (_currentView.position == -1) {
            ...
            for (int k = 0; k < j; k++) {
              final _AppointmentView _intersectView = _getAppointmentOnPosition(_currentView, intersectingAppointments);
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      ...
      double agendaHeight = widget.view == CalendarView.month && widget.monthViewSettings.showAgenda
          ? widget.monthViewSettings.agendaViewHeight
          : 0;
      if (agendaHeight == null || agendaHeight == -1) {
        agendaHeight = (_height + widget.headerHeight + _getViewHeaderHeight(widget.viewHeaderHeight, widget.view)) / 3;
      }
  ...
  Widget _addChildren(double _top, double agendaHeight, double _height, double _width, bool _isDark) {
    final DateTime _currentViewDate = _currentViewVisibleDates[(_currentViewVisibleDates.length / 2).truncate()];
    return Stack(children: <Widget>[
        ...
        child: GestureDetector(
          child: Container(
            ...
            child: CustomPaint(
              ...
              painter: _HeaderViewPainter(_currentViewVisibleDates, widget.headerStyle, _currentViewDate, widget.view,
                  widget.monthViewSettings.numberOfWeeksInView, _isDark),
            ),
          ),
      ...
      _addAgendaView(agendaHeight, _top + _height - agendaHeight, _width, _isDark),
    ]);
  }
  ...
  dynamic _updateCalendarState(_UpdateCalendarStateDetails details) {
    ...
    if (details._currentViewVisibleDates != null && _currentViewVisibleDates != details._currentViewVisibleDates) {
      _currentViewVisibleDates = details._currentViewVisibleDates;
      _updateVisibleAppointments();
      if (_shouldRaiseViewChangedCallback(widget.onViewChanged)) {
        _raiseViewChangedCallback(widget, visibleDates: _currentViewVisibleDates);
      }
    }
    ...
    if (details._allDayPanelHeight == null || details._allDayPanelHeight != _allDayPanelHeight) {
    ...
    if (details._appointments == null || details._appointments != _appointments) {
  ...
  void _updateCalendarTapCallbackForHeader() {
    ...
    if (widget.view == CalendarView.month) {
      selectedDate = DateTime(_currentDate.year, _currentDate.month, 01, 0, 0, 0);
    } else {
      selectedDate = DateTime(_currentViewVisibleDates[0].year, _currentViewVisibleDates[0].month,
          _currentViewVisibleDates[0].day, 0, 0, 0);
    }

    _raiseCalendarTapCallback(widget, date: selectedDate, element: CalendarElement.header);
  }
  ...
  void _updateCalendarTapCallbackForAgendaView(TapUpDetails details) {
    ...
    if (_selectedDate != null) {
      _agendaAppointments = _getSelectedDateAppointments(_appointments, widget.timeZone, _selectedDate);

      final double dateTimeWidth = _minWidth * 0.15;
      if (details.localPosition.dx > dateTimeWidth && _agendaAppointments != null && _agendaAppointments.isNotEmpty) {
        _agendaAppointments
            .sort((dynamic app1, dynamic app2) => app1._actualStartTime.compareTo(app2._actualStartTime));
        _agendaAppointments
            .sort((dynamic app1, dynamic app2) => _orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));

        int index = -1;
        //// Agenda appointment view top padding as 5.
        const double padding = 5;
        double xPosition = 0;
        final double tappedYPosition = _agendaScrollController.offset + details.localPosition.dy;
    ...
    if (widget.dataSource != null && !_isCalendarAppointment(widget.dataSource.appointments)) {
      _selectedAppointments = _getCustomAppointments(_agendaAppointments);
    }

    _raiseCalendarTapCallback(widget,
        date: _selectedDate, appointments: _selectedAppointments, element: CalendarElement.agenda);
  }

  // Returns the agenda view  as a child for the calendar.
  Widget _addAgendaView(double height, double startPosition, double _width, bool _isDark) {
    if (widget.view != CalendarView.month || !widget.monthViewSettings.showAgenda) {
      return Positioned(
        left: 0,
        right: 0,
    ...
    if (_agendaAppointments != null && _agendaAppointments.isNotEmpty) {
      ...
      painterHeight = (((count * ((appointmentHeight / 2) + topPadding)) +
                  ((_agendaAppointments.length - count) * (appointmentHeight + topPadding)))
              .toDouble()) +
          bottomPadding;
    }
                  ...
                  children: <Widget>[
                    CustomPaint(
                      painter: _AgendaDateTimePainter(
                          _selectedDate, widget.monthViewSettings, _isDark, widget.todayHighlightColor),
                        ...
                        children: <Widget>[
                          CustomPaint(
                            painter: _AgendaViewPainter(
                                widget.monthViewSettings, _selectedDate, widget.timeZone, _appointments),
