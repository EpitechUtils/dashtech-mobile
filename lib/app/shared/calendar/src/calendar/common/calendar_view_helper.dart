...
bool _isDarkTheme(BuildContext context) {
  ...
  return theme != null && theme.brightness != null && theme.brightness == Brightness.dark;
}
...
int _getTimeInterval(TimeSlotViewSettings settings) {
  ...
  } else if (settings.timeInterval.inMinutes >= 0 && settings.timeInterval.inMinutes <= totalMinutes) {
...
bool _isWithInVisibleDateRange(List<DateTime> visibleDates, DateTime selectedDate) {
  ...
  if (selectedDate != null) {
    ...
    {
      if (_isSameDate(firstDate, selectedDate) || _isSameDate(lastDate, selectedDate))
        return true;
      else
        return selectedDate.isAfter(firstDate) && selectedDate.isBefore(lastDate);
    }
  } else
    return false;
}
...
void _raiseViewChangedCallback(SfCalendar calendar, {List<DateTime> visibleDates}) {
...
double _getTimeIntervalHeight(
    SfCalendar calendar, double _width, double _height, int _visibleDatesCount, double _allDayHeight) {
  double _timeIntervalHeight = calendar.timeSlotViewSettings.timeIntervalHeight;
  double _viewHeaderHeight = _getViewHeaderHeight(calendar.viewHeaderHeight, calendar.view);

  if (calendar.view == CalendarView.day) {
    _allDayHeight = _kAllDayLayoutHeight;
    _viewHeaderHeight = 0;
  } else {
    _allDayHeight = _allDayHeight > _kAllDayLayoutHeight ? _kAllDayLayoutHeight : _allDayHeight;
  }

  if (!_isTimelineView(calendar.view)) {
    if (_isAutoTimeIntervalHeight(calendar)) {
      _timeIntervalHeight =
          (_height - _allDayHeight - _viewHeaderHeight) / _getHorizontalLinesCount(calendar.timeSlotViewSettings);
    }
  } else {
    if (_isAutoTimeIntervalHeight(calendar)) {
      if (calendar.view == CalendarView.day) {
        _timeIntervalHeight = _width / _getHorizontalLinesCount(calendar.timeSlotViewSettings);
      } else if (calendar.view != CalendarView.month) {
        final dynamic _horizontalLinesCount = _getHorizontalLinesCount(calendar.timeSlotViewSettings);
        _timeIntervalHeight = _width / (_horizontalLinesCount * _visibleDatesCount);
        if (!_isValidWidth(_width, calendar, _visibleDatesCount, _horizontalLinesCount)) {
...
bool _isValidWidth(double _screenWidth, SfCalendar calendar, int _visibleDatesCount, double _horizontalLinesCount) {
  const dynamic offSetValue = 10;
  final dynamic tempWidth = _visibleDatesCount * offSetValue * _horizontalLinesCount;
...
bool _isTimelineView(CalendarView view) {
  return view == CalendarView.timelineDay || view == CalendarView.timelineWeek || view == CalendarView.timelineWorkWeek;
}
