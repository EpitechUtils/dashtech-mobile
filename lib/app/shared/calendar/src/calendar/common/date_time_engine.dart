...
List<DateTime> _getVisibleDates(DateTime date, List<int> nonWorkingDays, int firstDayOfWeek, int visibleDatesCount) {
  final List<DateTime> datesCollection = <DateTime>[];
  DateTime currentDate = date;
  if (firstDayOfWeek != null) {
    currentDate = _getDateOnFirstDayOfWeek(visibleDatesCount, date, firstDayOfWeek);
  }
...
DateTime _getDateOnFirstDayOfWeek(int visibleDatesCount, DateTime date, int firstDayOfWeek) {
...
DateTime _getNextViewStartDate(CalendarView _view, DateTime date, int numberOfWeeks) {
  if (_view == CalendarView.month) {
    return numberOfWeeks == 6 ? _getNextMonthDate(date) : date.add(Duration(days: numberOfWeeks * 7));
  } else if (_view == CalendarView.week ||
      _view == CalendarView.workWeek ||
      _view == CalendarView.timelineWorkWeek ||
      _view == CalendarView.timelineWeek) {
...
DateTime _getPreviousViewStartDate(CalendarView _view, DateTime date, int numberOfWeeks) {
  if (_view == CalendarView.month) {
    return numberOfWeeks == 6 ? _getPreviousMonthDate(date) : date.add(Duration(days: -numberOfWeeks * 7));
  } else if (_view == CalendarView.week ||
      _view == CalendarView.workWeek ||
      _view == CalendarView.timelineWorkWeek ||
      _view == CalendarView.timelineWeek) {
...
DateTime _getPreviousMonthDate(DateTime date) {
  return date.month == 1 ? DateTime(date.year - 1, 12, 1) : DateTime(date.year, date.month - 1, 1);
}

//// Calculate the next month visible start date.
DateTime _getNextMonthDate(DateTime date) {
  return date.month == 12 ? DateTime(date.year + 1, 1, 1) : DateTime(date.year, date.month + 1, 1);
}
