...
class _CustomScrollView extends StatefulWidget {
  const _CustomScrollView(
      this._calendar, this._width, this._height, this._visibleAppointments, this._agendaSelectedDate,
...
class _CustomScrollViewState extends State<_CustomScrollView> with TickerProviderStateMixin {
  ...
  List<DateTime> _visibleDates, _previousViewVisibleDates, _nextViewVisibleDates, _currentViewVisibleDates;
  ...
  void initState() {
    ...
    _animation = _tween.animate(_animationController)..addListener(animationListener);
  ...
  void _updateVisibleDates() {
    ...
    final DateTime prevDate = _getPreviousViewStartDate(widget._calendar.view, _updateCalendarStateDetails._currentDate,
        widget._calendar.monthViewSettings.numberOfWeeksInView);
    final DateTime nextDate = _getNextViewStartDate(widget._calendar.view, _updateCalendarStateDetails._currentDate,
        widget._calendar.monthViewSettings.numberOfWeeksInView);
    final List<int> _nonWorkingDays =
        (widget._calendar.view == CalendarView.workWeek || widget._calendar.view == CalendarView.timelineWorkWeek)
            ? widget._calendar.timeSlotViewSettings.nonWorkingDays
            : null;

    _visibleDates = _getVisibleDates(currentDate, _nonWorkingDays, widget._calendar.firstDayOfWeek,
        _getViewDatesCount(widget._calendar.view, widget._calendar.monthViewSettings.numberOfWeeksInView, currentDate));
    _previousViewVisibleDates = _getVisibleDates(prevDate, _nonWorkingDays, widget._calendar.firstDayOfWeek,
        _getViewDatesCount(widget._calendar.view, widget._calendar.monthViewSettings.numberOfWeeksInView, prevDate));
    _nextViewVisibleDates = _getVisibleDates(nextDate, _nonWorkingDays, widget._calendar.firstDayOfWeek,
        _getViewDatesCount(widget._calendar.view, widget._calendar.monthViewSettings.numberOfWeeksInView, nextDate));

    _currentViewVisibleDates = _visibleDates;
    _updateCalendarStateDetails._currentViewVisibleDates = _currentViewVisibleDates;
    widget.updateCalendarState(_updateCalendarStateDetails);
  ...
  void _updateNextViewVisibleDates() {
    ...
    if (widget._calendar.view == CalendarView.month && widget._calendar.monthViewSettings.numberOfWeeksInView == 6) {
      _currentViewDate = _currentViewVisibleDates[(_currentViewVisibleDates.length / 2).truncate()];
    }

    _currentViewDate = _getNextViewStartDate(
        widget._calendar.view, _currentViewDate, widget._calendar.monthViewSettings.numberOfWeeksInView);
    final List<DateTime> _dates = _getVisibleDates(
        ...
        _getViewDatesCount(
            widget._calendar.view, widget._calendar.monthViewSettings.numberOfWeeksInView, _currentViewDate));
  ...
  void _updateCalendarViewStateDetails(_UpdateCalendarStateDetails details) {
    ...
    if (details._appointments == null || details._appointments != _updateCalendarStateDetails._appointments) {
  ...
  List<Widget> _addViews() {
    ...
    final dynamic previousView = _updateViews(_previousView, _previousViewKey, _previousViewVisibleDates);
    final dynamic currentView = _updateViews(_currentView, _currentViewKey, _visibleDates);
    final dynamic nextView = _updateViews(_nextView, _nextViewKey, _nextViewVisibleDates);
  ...
  _CalendarView _updateViews(_CalendarView view, GlobalKey<_CalendarViewState> _viewKey, List<DateTime> _visibleDates) {
    ...
    if (view._visibleDates != _visibleDates) {
      ...
      _children[index] = view;
        ...
        _viewKey.currentState._appointmentPainter._visibleAppointments != widget._visibleAppointments) {
  ...
  void didUpdateWidget(_CustomScrollView oldWidget) {
    ...
    if (oldWidget._calendar.timeSlotViewSettings != widget._calendar.timeSlotViewSettings ||
        oldWidget._calendar.monthViewSettings != widget._calendar.monthViewSettings ||
        oldWidget._calendar.viewHeaderStyle != widget._calendar.viewHeaderStyle ||
        oldWidget._calendar.viewHeaderHeight != widget._calendar.viewHeaderHeight ||
        oldWidget._calendar.todayHighlightColor != widget._calendar.todayHighlightColor ||
        oldWidget._calendar.cellBorderColor != widget._calendar.cellBorderColor ||
        oldWidget._calendar.selectionDecoration != widget._calendar.selectionDecoration) {
      _children.clear();
      _position = 0;
    }
    ...
    if (_isTimelineView(widget._calendar.view) != _isTimelineView(oldWidget._calendar.view)) {
      _children.clear();
    }

    /// position set as zero to maintain the existing scroll position in time line view
    if (_isTimelineView(widget._calendar.view) &&
        (oldWidget._calendar.backgroundColor != widget._calendar.backgroundColor ||
            oldWidget._calendar.headerStyle != widget._calendar.headerStyle) &&
        _position != null) {
      _position = 0;
    }

    if (oldWidget._calendar.initialDisplayDate != widget._calendar.initialDisplayDate &&
        widget._calendar.initialDisplayDate != null) {
      _updateCalendarStateDetails._currentDate = widget._calendar.initialDisplayDate;
      _updateVisibleDates();
      _updateMoveToDate();
    }

    if (oldWidget._calendar.initialSelectedDate != widget._calendar.initialSelectedDate) {
      _updateCalendarStateDetails._selectedDate = widget._calendar.initialSelectedDate;
      _updateSelection();
      _position = 0;
    }
  ...
  void _updateSelection() {
    ...
    previousViewState._selectionPainter._selectedDate = _updateCalendarStateDetails._selectedDate;
    nextViewState._selectionPainter._selectedDate = _updateCalendarStateDetails._selectedDate;
    currentViewState._selectionPainter._selectedDate = _updateCalendarStateDetails._selectedDate;
    ...
    currentViewState._selectionPainter._repaintNotifier.value =
        !currentViewState._selectionPainter._repaintNotifier.value;
    nextViewState._selectionPainter._repaintNotifier.value = !nextViewState._selectionPainter._repaintNotifier.value;
  }
  ...
  void _updateCurrentViewVisibleDates({bool isNextView = false}) {
    ...
    _updateCalendarStateDetails._currentViewVisibleDates = _currentViewVisibleDates;
    _updateCalendarStateDetails._currentDate = _currentViewVisibleDates[_currentViewVisibleDates.length ~/ 2];
    widget.updateCalendarState(_updateCalendarStateDetails);
  }
  ...
  void _resetPosition() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      if (_position.abs() == widget._width || _position.abs() == widget._height) {
        _position = 0;
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget._calendar.monthViewSettings.navigationDirection == MonthNavigationDirection.horizontal ||
      ...
      children: <Widget>[
        Positioned(
          ...
          child: GestureDetector(
            child: _CustomScrollViewerLayout(_addViews(), widget._calendar, _position, _currentChildIndex),
            ...
            onVerticalDragStart: (DragStartDetails details) {
              if (widget._calendar.view == CalendarView.month && !_isTimeline) _onVerticalStart(details);
            },
            onVerticalDragUpdate: (DragUpdateDetails details) {
              if (widget._calendar.view == CalendarView.month && !_isTimeline) _onVerticalUpdate(details);
            },
            onVerticalDragEnd: (DragEndDetails details) {
              if (widget._calendar.view == CalendarView.month && !_isTimeline) _onVerticalEnd(details);
            },
          ),
        )
      ],
  ...
  void _onHorizontalStart(DragStartDetails dragStartDetails) {
    ...
    if (_isTimelineView(widget._calendar.view)) {
      ...
      if (_currentChildIndex == 0) {
        _nextViewState._scrollController.jumpTo(_nextViewState._scrollController.position.maxScrollExtent);
        _currentViewState._scrollController.jumpTo(0);
      } else if (_currentChildIndex == 1) {
        _previousViewState._scrollController.jumpTo(_previousViewState._scrollController.position.maxScrollExtent);
        _nextViewState._scrollController.jumpTo(0);
      } else if (_currentChildIndex == 2) {
        _currentViewState._scrollController.jumpTo(_currentViewState._scrollController.position.maxScrollExtent);
    ...
    if (_isTimelineView(widget._calendar.view)) {
      ...
      for (int i = 0; i < _children.length; i++) {
        if (_children[i]._visibleDates == _currentViewVisibleDates) {
          ...
          if (viewKey.currentState
              ._isUpdateTimelineViewScroll(_scrollStartPosition, dragUpdateDetails.globalPosition.dx)) return;
          break;
        }
      }
    }
  ...
  void _onHorizontalEnd(DragEndDetails dragEndDetails) {
    ...
    if (_isTimelineView(widget._calendar.view)) {
      for (int i = 0; i < _children.length; i++) {
        if (_children[i]._visibleDates == _currentViewVisibleDates) {
          final GlobalKey<_CalendarViewState> viewKey = _children[i].key;
          if (viewKey.currentState._isAnimateTimelineViewScroll(_position, dragEndDetails.primaryVelocity)) {
            return;
          }
          break;
        }
    ...
    if (widget._calendar.monthViewSettings.navigationDirection == MonthNavigationDirection.horizontal ||
      ...
      // fling the view from right to left
        ...
        // Resets the controller to forward it again, the animation will forward only from the dismissed state
        if (_animationController.isCompleted && _position != _tween.end) _animationController.reset();
  ...
  void _onVerticalStart(DragStartDetails dragStartDetails) {
    if (widget._calendar.monthViewSettings.navigationDirection == MonthNavigationDirection.vertical &&
  ...
  void _clearSelection() {
    ...
    for (int i = 0; i < _children.length; i++) {
      final GlobalKey<_CalendarViewState> viewKey = _children[i].key;
      if (viewKey.currentState._selectionPainter._selectedDate != _updateCalendarStateDetails._selectedDate) {
        viewKey.currentState._selectionPainter._selectedDate = _updateCalendarStateDetails._selectedDate;
