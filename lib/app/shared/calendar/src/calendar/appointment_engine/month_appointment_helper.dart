...
_AppointmentView _getAppointmentView(Appointment appointment, _AppointmentPainter _appointmentPainter) {
...
void _createVisibleAppointments(_AppointmentPainter _appointmentPainter) {
  for (int i = 0; i < _appointmentPainter._appointmentCollection.length; i++) {
    final _AppointmentView appointmentView = _appointmentPainter._appointmentCollection[i];
  ...
  for (int i = 0; i < _appointmentPainter._visibleAppointments.length; i++) {
    ...
    if (!appointment._isSpanned &&
        ...
        appointment._actualStartTime.month == appointment._actualEndTime.month) {
      final _AppointmentView appointmentView = _createAppointmentView(_appointmentPainter);
      appointmentView.appointment = appointment;
      appointmentView.canReuse = false;
      appointmentView.startIndex = _getDateIndex(appointment._actualStartTime, _appointmentPainter);
      if (appointmentView.startIndex == -1) {
        appointmentView.startIndex = 0;
      }

      appointmentView.endIndex = _getDateIndex(appointment._actualEndTime, _appointmentPainter);
      ...
      _createAppointmentInfoForSpannedAppointment(appointmentView, _appointmentPainter);
    }
  }
}
...
void _createAppointmentInfoForSpannedAppointment(
  ...
  if (appointmentView.startIndex ~/ 7 != appointmentView.endIndex ~/ 7) {
    ...
    appointmentView.endIndex = ((((appointmentView.startIndex ~/ 7) + 1) * 7) - 1).toInt();
    ...
    final _AppointmentView appointmentView1 = _createAppointmentView(_appointmentPainter);
    ...
    _createAppointmentInfoForSpannedAppointment(appointmentView1, _appointmentPainter);
  } else {
    appointmentView.isSpanned = true;
    if (!_appointmentPainter._appointmentCollection.contains(appointmentView)) {
      _appointmentPainter._appointmentCollection.add(appointmentView);
...
int _orderAppointmentViewBySpanned(_AppointmentView _appointmentView1, _AppointmentView _appointmentView2) {
  final int boolValue1 = _appointmentView1.isSpanned ? -1 : 1;
  final int boolValue2 = _appointmentView2.isSpanned ? -1 : 1;

  if (boolValue1 == boolValue2 && _appointmentView2.startIndex == _appointmentView1.startIndex) {
...
void _setAppointmentPosition(
    List<_AppointmentView> appointmentViewCollection, _AppointmentView appointmentView, int viewIndex) {
...
bool _isInterceptAppointments(_AppointmentView appointmentView1, _AppointmentView appointmentView2) {
...
void _updateAppointmentPosition(_AppointmentPainter _appointmentPainter) {
  _appointmentPainter._appointmentCollection.sort((_AppointmentView app1, _AppointmentView app2) {
    if (app1.appointment?._actualStartTime != null && app2.appointment?._actualStartTime != null) {
      return app1.appointment._actualStartTime.compareTo(app2.appointment._actualStartTime);
    }

    return 0;
  });
  _appointmentPainter._appointmentCollection
      .sort((_AppointmentView app1, _AppointmentView app2) => _orderAppointmentViewBySpanned(app1, app2));

  for (int j = 0; j < _appointmentPainter._appointmentCollection.length; j++) {
    final _AppointmentView appointmentView = _appointmentPainter._appointmentCollection[j];
    appointmentView.position = 1;
    appointmentView.maxPositions = 1;
    _setAppointmentPosition(_appointmentPainter._appointmentCollection, appointmentView, j);
  }
}

int _getDateIndex(DateTime date, _AppointmentPainter _appointmentPainter) {
  DateTime dateTime = _appointmentPainter._visibleDates[_appointmentPainter._visibleDates.length - 7];
  int row = 0;
  ...
  while (dateTime.isBefore(endDateTime) ||
      (dateTime.day == endDateTime.day && dateTime.month == endDateTime.month && dateTime.year == endDateTime.year)) {
...
_AppointmentView _createAppointmentView(_AppointmentPainter _appointmentPainter) {
...
void _updateAppointment(_AppointmentPainter _appointmentPainter) {
  ...
  if (_appointmentPainter._visibleAppointments != null && _appointmentPainter._visibleAppointments.isNotEmpty) {
