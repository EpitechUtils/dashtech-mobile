...
bool _isCalendarAppointment(List<dynamic> dataSource) {
  if (dataSource == null || dataSource.isEmpty || dataSource[0].runtimeType.toString() == 'Appointment') {
    return true;
  }
...
bool _isSpanned(Appointment appointment) {
  return !(appointment._actualEndTime.day == appointment._actualStartTime.day &&
          appointment._actualEndTime.month == appointment._actualStartTime.month &&
          appointment._actualEndTime.year == appointment._actualStartTime.year) &&
      appointment._actualEndTime.difference(appointment._actualStartTime).inDays > 0;
}
...
List<Appointment> _getSelectedDateAppointments(List<Appointment> appointments, String timeZone, DateTime date) {
  ...
  for (int j = 0; j < count; j++) {
    ...
    appointment._actualStartTime =
        _convertTimeToAppointmentTimeZone(appointment.startTime, appointment.startTimeZone, timeZone);
    appointment._actualEndTime =
        _convertTimeToAppointmentTimeZone(appointment.endTime, appointment.endTimeZone, timeZone);
...
Appointment _copy(Appointment appointment) {
  ...
  _copyAppointment.recurrenceExceptionDates = appointment.recurrenceExceptionDates;
...
bool _isAppointmentWithinVisibleDateRange(Appointment appointment, DateTime visibleStart, DateTime visibleEnd) {
...
bool _isAppointmentInVisibleDateRange(Appointment appointment, DateTime visibleStart, DateTime visibleEnd) {
  final DateTime appStartTime = appointment._actualStartTime;
  final DateTime appEndTime = appointment._actualEndTime;
  if ((appStartTime.isAfter(visibleStart) ||
...
double _timeToPosition(SfCalendar calendar, DateTime date, double _timeIntervalHeight) {
  final double singleIntervalHeightForAnHour =
      ((60 / _getTimeInterval(calendar.timeSlotViewSettings)) * _timeIntervalHeight).toDouble();
  ...
  return ((hour + (minute / 60).toDouble() + (seconds / 3600).toDouble()) * singleIntervalHeightForAnHour) -
      (startHour * singleIntervalHeightForAnHour).toDouble();
}

double _getAppointmentHeightFromDuration(Duration minimumDuration, SfCalendar calendar, double _timeIntervalHeight) {
  if (minimumDuration == null || minimumDuration.inMinutes <= 0) {
    return 0;
  }

  final double hourHeight = ((60 / _getTimeInterval(calendar.timeSlotViewSettings)) * _timeIntervalHeight).toDouble();
  return minimumDuration.inMinutes * (hourHeight / 60);
}

double _getAppointmentMinHeight(SfCalendar calendar, _AppointmentView appView, double _timeIntervalHeight) {
  double minHeight;

  // Appointment Default Bottom Position without considering MinHeight
  final double defaultAppHeight = _timeToPosition(calendar, appView.appointment._actualEndTime, _timeIntervalHeight) -
      _timeToPosition(calendar, appView.appointment._actualStartTime, _timeIntervalHeight);

  minHeight = _getAppointmentHeightFromDuration(
      calendar.timeSlotViewSettings.minimumAppointmentDuration, calendar, _timeIntervalHeight);

  // Appointment Default Bottom Position - Default value as double.NaN
  if (minHeight == 0) {
    return defaultAppHeight;
  } else if ((minHeight < defaultAppHeight) || (_timeIntervalHeight < defaultAppHeight)) {
...
bool _isIntersectingAppointmentInDayView(SfCalendar calendar, Appointment currentApp, _AppointmentView appView,
    Appointment appointment, bool isAllDay, double _timeIntervalHeight) {
  ...
  if (_isDateTimeEqual(currentApp._actualStartTime, appointment._actualStartTime) ||
  ...
  if (calendar.timeSlotViewSettings.minimumAppointmentDuration != null &&
    ...
    if (!_isDateEqual(currentApp._actualStartTime, appointment._actualStartTime)) {
      return false;
    }

    // Comparing appointments rendered in the same date
    final double appTopPos = _timeToPosition(calendar, appointment._actualStartTime, _timeIntervalHeight);
    final double currentAppTopPos = _timeToPosition(calendar, currentApp._actualStartTime, _timeIntervalHeight);
    final double appHeight = _getAppointmentMinHeight(calendar, appView, _timeIntervalHeight);
...
_AppointmentView _getAppointmentOnPosition(_AppointmentView currentView, List<_AppointmentView> views) {
...
void _setAppointmentPositionAndMaxPosition(Object parent, SfCalendar calendar, List<Appointment> visibleAppointments,
    bool isAllDay, double _timeIntervalHeight) {
  if (visibleAppointments == null) {
    return;
  }

  final List<Appointment> normalAppointments =
      visibleAppointments.where((Appointment app) => _iterateAppointment(app, calendar, isAllDay)).toList();
  normalAppointments
      .sort((Appointment app1, Appointment app2) => app1._actualStartTime.compareTo(app2._actualStartTime));
  if (!_isTimelineView(calendar.view)) {
    normalAppointments
        .sort((Appointment app1, Appointment app2) => _orderAppointmentsDescending(app1._isSpanned, app2._isSpanned));
    normalAppointments
        .sort((Appointment app1, Appointment app2) => _orderAppointmentsDescending(app1.isAllDay, app2.isAllDay));
  } else {
    normalAppointments
        .sort((Appointment app1, Appointment app2) => _orderAppointmentsAscending(app1._isSpanned, app2._isSpanned));
    normalAppointments
        .sort((Appointment app1, Appointment app2) => _orderAppointmentsAscending(app1.isAllDay, app2.isAllDay));
  }
  ...
  for (int count = 0; count < normalAppointments.length; count++) {
    ...
    if ((calendar.view == CalendarView.workWeek || calendar.view == CalendarView.timelineWorkWeek) &&
        calendar.timeSlotViewSettings.nonWorkingDays.contains(currentAppointment._actualStartTime.weekday) &&
        calendar.timeSlotViewSettings.nonWorkingDays.contains(currentAppointment._actualEndTime.weekday)) {
      continue;
    }
    ...
    for (int position = 0; position < maxColsCount; position++) {
      ...
      for (int j = 0; j < processedViews.length; j++) {
        ...
        if (_isIntersectingAppointmentInDayView(
            calendar, currentAppointment, previousApp, previousApp.appointment, isAllDay, _timeIntervalHeight)) {
          isIntersecting = true;
    ...
    if (currentAppView.position == -1) {
      ...
      if (intersectingApps == null) {
        ...
        position = intersectingApps
            .reduce((_AppointmentView currentAppview, _AppointmentView nextAppview) =>
...
DateTime _convertTimeToAppointmentTimeZone(DateTime date, String appTimeZoneId, String calendarTimeZoneId) {
  ...
  if (appTimeZoneId != null && appTimeZoneId != '') {
    ...
    if (appTimeZoneId == 'Dateline Standard Time') {
      ...
      convertedDate = TZDateTime.from(date, _timeZoneInfoToOlsonTimeZone(appTimeZoneId));
    }
  ...
  if (calendarTimeZoneId != null && calendarTimeZoneId != '') {
    ...
    if (calendarTimeZoneId == 'Dateline Standard Time') {
      ...
      actualConvertedDate = TZDateTime.from(convertedDate, _timeZoneInfoToOlsonTimeZone(calendarTimeZoneId));
    }
...
List<Appointment> _getVisibleAppointments(DateTime visibleStartDate, DateTime visibleEndDate,
    List<Appointment> appointments, String calendarTimeZone, bool isTimelineView) {
  final List<Appointment> appointmentColl = <Appointment>[];
  if (visibleStartDate == null || visibleEndDate == null || appointments == null) {
  ...
  for (int j = 0; j < count; j++) {
    final Appointment appointment = appointments[j];
    appointment._actualStartTime =
        _convertTimeToAppointmentTimeZone(appointment.startTime, appointment.startTimeZone, calendarTimeZone);
    appointment._actualEndTime =
        _convertTimeToAppointmentTimeZone(appointment.endTime, appointment.endTimeZone, calendarTimeZone);
    ...
    if (appointment.recurrenceRule == null || appointment.recurrenceRule == '') {
      if (_isAppointmentWithinVisibleDateRange(appointment, startDate, endDate)) {
        ...
        if (!(appStartTime.day == appEndTime.day &&
          ...
          for (int i = 0; i < 2; i++) {
            ...
            if (i == 0) {
              spannedAppointment._actualEndTime =
                  DateTime(appStartTime.year, appStartTime.month, appStartTime.day, 23, 59, 59);
            } else {
              spannedAppointment._actualStartTime =
                  DateTime(appEndTime.year, appEndTime.month, appEndTime.day, 0, 0, 0);
            }

            spannedAppointment.startTime = spannedAppointment.isAllDay
                ? appointment._actualStartTime
                : _convertTimeToAppointmentTimeZone(
                    appointment._actualStartTime, appointment.startTimeZone, calendarTimeZone);
            spannedAppointment.endTime = spannedAppointment.isAllDay
                ? appointment._actualEndTime
                : _convertTimeToAppointmentTimeZone(
                    appointment._actualEndTime, appointment.endTimeZone, calendarTimeZone);

            // Adding Spanned Appointment only when the Appointment range within the VisibleDate
            if (_isAppointmentWithinVisibleDateRange(spannedAppointment, startDate, endDate)) {
          ...
          if (_isAppointmentInVisibleDateRange(appointment, startDate, endDate)) {
          ...
          } else if (_isAppointmentStartDateWithinVisibleDateRange(appointment._actualStartTime, startDate, endDate)) {
            ...
            for (int i = 0; i < 2; i++) {
              final Appointment spannedAppointment = _copy(appointment);
              if (i == 0) {
                spannedAppointment._actualEndTime = DateTime(endDate.year, endDate.month, endDate.day, 23, 59, 59);
              } else {
                spannedAppointment._actualStartTime = DateTime(endDate.year, endDate.month, endDate.day, 0, 0, 0);
              }
          ...
          } else if (_isAppointmentEndDateWithinVisibleDateRange(appointment._actualEndTime, startDate, endDate)) {
            ...
            for (int i = 0; i < 2; i++) {
              final Appointment spannedAppointment = _copy(appointment);
              if (i == 0) {
                spannedAppointment._actualStartTime = appointment._actualStartTime;
                final DateTime date = startDate.add(const Duration(days: -1));
                spannedAppointment._actualEndTime = DateTime(date.year, date.month, date.day, 23, 59, 59);
              } else {
                spannedAppointment._actualStartTime = DateTime(startDate.year, startDate.month, startDate.day, 0, 0, 0);
              }
              ...
              !_isAppointmentStartDateWithinVisibleDateRange(appointment._actualStartTime, startDate, endDate)) {
            ...
            for (int i = 0; i < 3; i++) {
              ...
              if (i == 0) {
                ...
                spannedAppointment._actualStartTime = DateTime(date.year, date.month, date.day, 0, 0, 0);
              }
    ...
    _getRecurrenceAppointments(appointment, appointmentColl, startDate, endDate, calendarTimeZone);
  }

  return appointmentColl;
}

Appointment _cloneRecurrenceAppointment(
    Appointment appointment, int recurrenceIndex, DateTime recursiveDate, String calendarTimeZone) {
...
//// Recursive DataContext has been set based on custom data for AppointmentTemplate added.
  ...
  occurrenceAppointment.startTime = occurrenceAppointment.isAllDay
      ...
      : _convertTimeToAppointmentTimeZone(
          occurrenceAppointment._actualStartTime, occurrenceAppointment.startTimeZone, calendarTimeZone);

  final int minutes = appointment._actualEndTime.difference(appointment._actualStartTime).inMinutes;
  occurrenceAppointment._actualEndTime = occurrenceAppointment._actualStartTime.add(Duration(minutes: minutes));
  occurrenceAppointment.endTime = occurrenceAppointment.isAllDay
      ? occurrenceAppointment._actualEndTime
      : _convertTimeToAppointmentTimeZone(
          occurrenceAppointment._actualEndTime, occurrenceAppointment.endTimeZone, calendarTimeZone);
  occurrenceAppointment._isSpanned = _isSpanned(occurrenceAppointment) &&
      (occurrenceAppointment.endTime.difference(occurrenceAppointment.startTime).inDays > 0);

  return occurrenceAppointment;
}

List<Appointment> _generateCalendarAppointments(CalendarDataSource calendarData, SfCalendar calendar,
  ...
  if (dataSource.isNotEmpty && dataSource[0].runtimeType.toString() == 'Appointment') {
    for (int i = 0; i < dataSource.length; i++) {
      ...
      item._actualStartTime = !item.isAllDay
          ? _convertTimeToAppointmentTimeZone(item.startTime, item.startTimeZone, calendar.timeZone)
          : item.startTime;
      item._actualEndTime = !item.isAllDay
          ? _convertTimeToAppointmentTimeZone(item.endTime, item.endTimeZone, calendar.timeZone)
          : item.endTime;
      _updateTimeForInvalidEndTime(item, calendar.timeZone);
      calendarAppointmentCollection.add(item);

      item._isSpanned = _isSpanned(item) && (appEndTime.difference(appStartTime).inDays > 0);
    }
  } else {
    for (int i = 0; i < dataSource.length; i++) {
      ...
      app._isSpanned = _isSpanned(app) && (appEndTime.difference(appStartTime).inDays > 0);
...
Appointment _createAppointment(Object appointmentObject, SfCalendar calendar) {
  ...
  app.recurrenceExceptionDates = calendar.dataSource.getRecurrenceExceptionDates(index);
  app._data = appointmentObject;
  app._actualStartTime = !app.isAllDay
      ? _convertTimeToAppointmentTimeZone(app.startTime, app.startTimeZone, calendar.timeZone)
      : app.startTime;
  app._actualEndTime =
      !app.isAllDay ? _convertTimeToAppointmentTimeZone(app.endTime, app.endTimeZone, calendar.timeZone) : app.endTime;
  _updateTimeForInvalidEndTime(app, calendar.timeZone);
  return app;
}

void _updateTimeForInvalidEndTime(Appointment appointment, String scheduleTimeZone) {
  if (appointment._actualEndTime.isBefore(appointment._actualStartTime) && !appointment.isAllDay) {
    appointment.endTime = _convertTimeToAppointmentTimeZone(
        appointment._actualStartTime.add(const Duration(minutes: 30)), appointment.endTimeZone, scheduleTimeZone);
    appointment._actualEndTime = !appointment.isAllDay
        ? _convertTimeToAppointmentTimeZone(appointment.endTime, appointment.endTimeZone, scheduleTimeZone)
        : appointment.endTime;
  }
}

void _getRecurrenceAppointments(Appointment appointment, List<Appointment> appointments, DateTime visibleStartDate,
    DateTime visibleEndDate, String scheduleTimeZone) {
  final DateTime appStartTime = appointment._actualStartTime;
  ...
  final List<String> rrule = _splitRule(appointment.recurrenceRule, ruleSeparator);
  if (appointment.recurrenceRule.contains('UNTIL')) {
    ...
    recursiveDates = _getRecurrenceDateTimeCollection(appointment.recurrenceRule, appointment._actualStartTime);
    endDate = recursiveDates.last;

    endDate = DateTime(endDate.year, endDate.month, endDate.day, 23, 59, 59);
  }

  if ((appointment.recurrenceRule.contains('UNTIL') || appointment.recurrenceRule.contains('COUNT')) &&
      !(appStartTime.isBefore(visibleEndDate) && visibleStartDate.isBefore(endDate))) {
    return;
  }

  recursiveDates = _getRecurrenceDateTimeCollection(rule, appointment._actualStartTime,
  ...
  for (int j = 0; j < recursiveDates.length; j++) {
    ...
    if (appointment.recurrenceExceptionDates != null) {
      ...
      for (int i = 0; i < appointment.recurrenceExceptionDates.length; i++) {
        final DateTime date =
            _convertTimeToAppointmentTimeZone(appointment.recurrenceExceptionDates[i], '', scheduleTimeZone);
    ...
    final Appointment occurrenceAppointment =
        _cloneRecurrenceAppointment(appointment, recurrenceIndex, recursiveDate, scheduleTimeZone);
    recurrenceIndex++;
