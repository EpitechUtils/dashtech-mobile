...
List<DateTime> _getRecurrenceDateTimeCollection(String rRule, DateTime recurrenceStartDate,
  ...
  final dynamic isSpecificDateRange = specificStartDate != null && specificEndDate != null;
  ...
  if (ruleArray.isNotEmpty && rRule != null && rRule != '') {
    ...
    if (rRule.contains('UNTIL')) {
      ...
      if (isSpecificDateRange) {
        endDate = (endDate.isAfter(specificEndDate) || endDate == specificEndDate) ? specificEndDate : endDate;
      }
    }
...
//// NoEndDate specified rule returns empty collection issue fix.
    ...
    if (daily == 'DAILY') {
      if (!rRule.contains('BYDAY')) {
        final int dailyDayGap = !rRule.contains('INTERVAL') ? 1 : int.parse(intervalCount);
        int tempcount = 0;
        while (tempcount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
          if (isSpecificDateRange) {
            if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
                (addDate.isBefore(specificEndDate) || addDate == specificEndDate)) {
        ...
        while (recDateCollection.length < recCount ||
          ...
          if (addDate.weekday != DateTime.sunday && addDate.weekday != DateTime.saturday) {
            if (isSpecificDateRange) {
              if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
                  (addDate.isBefore(specificEndDate) || addDate == specificEndDate)) {
                recDateCollection.add(addDate);
      ...
      int tempCount = 0;
      final int weeklyWeekGap = ruleArray.length > 4 && interval == 'INTERVAL' ? int.parse(intervalCount) : 1;
      ...
      while ((tempCount < recCount && isweeklyselected) ||
        ...
        if (isSpecificDateRange) {
          if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
              (addDate.isBefore(specificEndDate) || addDate == specificEndDate)) {
            _setWeeklyRecurrenceDate(addDate, weeklyRule, weeklyByDayPos, recDateCollection);
          }
      ...
      final int monthlyMonthGap = ruleArray.length > 4 && interval == 'INTERVAL' ? int.parse(intervalCount) : 1;
      if (byMonthDay == 'BYMONTHDAY') {
        ...
        if (monthDate < 30) {
          ...
          final DateTime temp =
              DateTime(addDate.year, addDate.month, monthDate, addDate.hour, addDate.minute, addDate.second);
          addDate = monthDate < currDate
              ? DateTime(temp.year, temp.month + 1, temp.day, temp.hour, temp.minute, temp.second)
              : temp;
          int tempcount = 0;
          while (tempcount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
            if (addDate.month == 2 && monthDate > 28) {
              addDate = DateTime(
                  addDate.year,
                  addDate.month,
                  DateTime(addDate.year, addDate.month + 1, 1).subtract(const Duration(days: 1)).day,
                  addDate.hour,
                  addDate.minute,
                  addDate.second);
              if (isSpecificDateRange) {
              ...
              addDate = DateTime(addDate.year, addDate.month + monthlyMonthGap, monthDate, addDate.hour, addDate.minute,
                  addDate.second);
            } else {
              if (isSpecificDateRange) {
                if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
              ...
              addDate = DateTime(addDate.year, addDate.month + monthlyMonthGap, addDate.day, addDate.hour,
          ...
          while (tempcount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
            ...
            addDate = DateTime(
                ...
                DateTime(addDate.year, addDate.month + monthlyMonthGap, 1).subtract(const Duration(days: 1)).day,
                addDate.hour,
                addDate.minute,
                addDate.second);
            tempcount++;
        ...
        while (recDateCollection.length < recCount ||
          ...
          final DateTime monthStart =
              DateTime(addDate.year, addDate.month, 1, addDate.hour, addDate.minute, addDate.second);
          final DateTime weekStartDate = monthStart.add(Duration(days: -monthStart.weekday));
          ...
          if (addDate.isBefore(recurrenceStartDate)) {
            addDate =
                DateTime(addDate.year, addDate.month + 1, addDate.day, addDate.hour, addDate.minute, addDate.second);
            continue;
          }
          ...
          if (isSpecificDateRange) {
            if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
                (addDate.isBefore(specificEndDate) || addDate == specificEndDate) &&
                (tempRecDateCollectionCount < recCount || rRule.contains('UNTIL'))) {
              recDateCollection.add(addDate);
            }
          ...
          addDate = DateTime(addDate.year, addDate.month + monthlyMonthGap, addDate.day - (addDate.day - 1),
              addDate.hour, addDate.minute, addDate.second);
          tempRecDateCollectionCount++;
        }
      }
    } else if (yearly == 'YEARLY') {
      final int yearlyYearGap = ruleArray.length > 4 && interval == 'INTERVAL' ? int.parse(intervalCount) : 1;
      if (byMonthDay == 'BYMONTHDAY') {
        final int monthIndex = int.parse(byMonthCount);
        final int dayIndex = int.parse(byMonthDayCount);
        if (monthIndex > 0 && monthIndex <= 12) {
          final int bound = DateTime(addDate.year, addDate.month + 1, 1).subtract(const Duration(days: 1)).day;
          if (bound >= dayIndex) {
            final DateTime specificDate =
                DateTime(addDate.year, monthIndex, dayIndex, addDate.hour, addDate.minute, addDate.second);
            if (specificDate.isBefore(addDate)) {
              addDate = specificDate;
              addDate =
                  DateTime(addDate.year + 1, addDate.month, addDate.day, addDate.hour, addDate.minute, addDate.second);
              if (isSpecificDateRange) {
                if ((addDate.isAfter(specificStartDate) || addDate == specificStartDate) &&
                    (addDate.isBefore(specificEndDate) || addDate == specificEndDate)) {
                  recDateCollection.add(addDate);
            ...
            while (tempcount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
              ...
              addDate = DateTime(addDate.year + yearlyYearGap, addDate.month, addDate.day, addDate.hour, addDate.minute,
                  addDate.second);
              tempcount++;
            }
          }
        ...
        final int monthIndex = int.parse(byMonthCount);
        DateTime monthStart = DateTime(addDate.year, monthIndex, 1, addDate.hour, addDate.minute, addDate.second);
        DateTime weekStartDate = monthStart.add(Duration(days: -monthStart.weekday));
        ...
        if ((monthStart.month != addDate.month && monthStart.isBefore(addDate)) ||
            (monthStart.month == addDate.month &&
                (monthStart.isBefore(addDate) || monthStart.isBefore(recurrenceStartDate)))) {
          addDate =
              DateTime(addDate.year + 1, addDate.month, addDate.day, addDate.hour, addDate.minute, addDate.second);
          monthStart = DateTime(addDate.year, monthIndex, 1, addDate.hour, addDate.minute, addDate.second);
        ...
        while (tempcount < recCount || (endDate != null && (addDate.isBefore(endDate) || addDate == endDate))) {
          ...
          addDate = DateTime(
              addDate.year + yearlyYearGap, addDate.month, addDate.day, addDate.hour, addDate.minute, addDate.second);
  ...
  if (ruleArray.isNotEmpty && (rRule != null || rRule != '')) {
    ...
    if (daily == 'DAILY') {
      ...
      if (rRule.contains('COUNT')) {
        ...
        recProp.interval = ruleArray.length > 4 && interval == 'INTERVAL' ? int.parse(intervalCount) : 1;
      }
    } else if (weekly == 'WEEKLY') {
      ...
      if (!rRule.contains('COUNT')) {
        ...
        recProp.interval = ruleArray.length > 4 && interval == 'INTERVAL' ? int.parse(intervalCount) : 1;
        int position = ruleArray.length > 4 && interval == 'INTERVAL' ? 6 : byDayPosition;
...
String _generateRRule(RecurrenceProperties recurrenceProperties, DateTime appStartTime, DateTime appEndTime) {
  ...
  if (recurrenceProperties.recurrenceType == RecurrenceType.daily) {
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      while ((count < recCount &&
          ...
          (recurrenceProperties.recurrenceRange == RecurrenceRange.noEndDate && i < 7)) {
        switch (addDate.weekday) {
          case DateTime.sunday:
            {
        ...
        addDate = addDate.weekday == DateTime.saturday
            ? addDate.add(Duration(days: ((recurrenceProperties.interval - 1) * 7) + 1))
            : addDate.add(const Duration(days: 1));
        i = i + 1;
      }
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      if (recurrenceProperties.week == -1) {
        rRule = rRule + ';BYMONTHDAY=' + recurrenceProperties.dayOfMonth.toString();
      } else {
        final DateTime firstDate = DateTime.now().subtract(Duration(days: DateTime.now().weekday - 1));
        final dynamic dayNames = List<dynamic>.generate(7, (dynamic index) => index)
            .map((dynamic value) => DateFormat(DateFormat.ABBR_WEEKDAY).format(firstDate.add(Duration(days: value))))
            .toList();
      ...
      if (DateTime(startDate.year, startDate.month + recurrenceProperties.interval, startDate.day, startDate.hour,
    ...
    if ((recCount > 0 && recurrenceProperties.recurrenceRange == RecurrenceRange.count) ||
      ...
      if (DateTime(startDate.year + recurrenceProperties.interval, startDate.month, startDate.day, startDate.hour,
                  startDate.minute, startDate.second)
              .difference(startDate) <
          diffTimeSpan) {
        isValidRecurrence = false;
...
bool _isRecurrenceDate(DateTime addDate, List<String> weeklyRule, int weeklyByDayPos) {
...
void _setWeeklyRecurrenceDate(
    DateTime addDate, List<String> weeklyRule, int weeklyByDayPos, List<DateTime> recDateCollection) {
  switch (addDate.weekday) {
    case DateTime.sunday:
      {
        if (weeklyRule[weeklyByDayPos].contains('SU')) {
